...
$app->group('/evento/', function () {
    ...
    $this->get('search/{query}', function($req, $res, $args) use ($modelEntity){
        ApiRenderer::jsonResponse($res,200,$modelEntity::where('nombre_evento', '=', $args['query'])->get()->toJson());
    });
    ...
    $this->post('', function($req, $res,$args) use ($app,$modelEntity) {
        try {
            ...
            $model->id_tipo_evento  = $allPostPutVars['id_tipo_evento'];
            $model->nombre_evento   = $allPostPutVars['nombre_evento'];
            $model->fecha           = $allPostPutVars['fecha'];
